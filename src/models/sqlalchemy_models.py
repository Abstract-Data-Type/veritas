...
from sqlalchemy import Column, Integer, String, DateTime, Text, ForeignKey, Float, CheckConstraint
...
class Article(Base):
    ...
    article_id: Mapped[int] = mapped_column("article_id", Integer, primary_key=True, index=True, autoincrement=True)
    ...
    interactions: Mapped[list["UserInteraction"]] = relationship("UserInteraction", back_populates="article")
...
class Summary(Base):
    ...
    summary_id: Mapped[int] = mapped_column("summary_id", Integer, primary_key=True, index=True, autoincrement=True)
...
class BiasRating(Base):
    ...
    rating_id: Mapped[int] = mapped_column("rating_id", Integer, primary_key=True, index=True, autoincrement=True)
    article_id: Mapped[int] = mapped_column(ForeignKey("articles.article_id"), nullable=False)
...
class UserInteraction(Base):
    ...
    __table_args__ = (
        CheckConstraint("action IN ('viewed', 'liked', 'bookmarked')", name="check_action"),
    )

    interaction_id: Mapped[int] = mapped_column("interaction_id", Integer, primary_key=True, index=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.user_id"), nullable=False)
    article_id: Mapped[int] = mapped_column(ForeignKey("articles.article_id"), nullable=False)
    action: Mapped[str] = mapped_column(String, nullable=False)
    interacted_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
