...
def get_all_bias_ratings(conn: Connection) -> List[Dict[str, Any]]:
    ...
    for rating in results:
        if rating['evaluated_at'] and isinstance(rating['evaluated_at'], str):
            try:
                rating['evaluated_at'] = datetime.fromisoformat(rating['evaluated_at'].replace('Z', '+00:00'))
            except ValueError:
                # Handle different datetime formats
                rating['evaluated_at'] = datetime.strptime(rating['evaluated_at'], '%Y-%m-%d %H:%M:%S')
...
def get_bias_rating_by_id(conn: Connection, rating_id: int) -> Optional[Dict[str, Any]]:
    ...
    if result and result['evaluated_at'] and isinstance(result['evaluated_at'], str):
        try:
            result['evaluated_at'] = datetime.fromisoformat(result['evaluated_at'].replace('Z', '+00:00'))
        except ValueError:
            # Handle different datetime formats
            result['evaluated_at'] = datetime.strptime(result['evaluated_at'], '%Y-%m-%d %H:%M:%S')
    
    return result


def update_bias_rating(conn: Connection, rating_id: int, bias_score: Optional[float] = None, 
...
def create_bias_rating(conn: Connection, article_id: int, bias_score: Optional[float] = None, 
