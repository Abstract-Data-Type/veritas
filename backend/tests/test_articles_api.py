...
class TestArticleByUrlEndpoint:
    """Test suite for /articles/by-url endpoint"""

    def test_get_article_by_url_success(self, client, sample_articles):
        """Test successful retrieval of article by URL"""
        from urllib.parse import quote
        
        url = "https://example.com/tech1"
        response = client.get(f"/articles/by-url?url={quote(url)}")

        assert response.status_code == 200
        data = response.json()

        assert data["title"] == "Tech News Update"
        assert data["url"] == url
        assert data["source"] == "TechDaily"
        assert "bias_rating" in data

    def test_get_article_by_url_not_found(self, client, sample_articles):
        """Test 404 when article URL doesn't exist"""
        from urllib.parse import quote
    ...
    def test_get_article_by_url_with_special_chars(self, client, test_db):
        """Test retrieval with URL containing special characters"""
        from urllib.parse import quote
        
        TestingSessionLocal, _ = test_db
        db = TestingSessionLocal()
        
        # Create article with special chars in URL
        special_url = "https://example.com/article?id=123&category=news"
        article = Article(
        ...
        response = client.get(f"/articles/by-url?url={quote(special_url)}")

        assert response.status_code == 200
        assert response.json()["url"] == special_url
