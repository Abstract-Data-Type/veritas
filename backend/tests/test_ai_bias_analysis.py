...
try:
    from veritas_news.ai import bias_analysis, config, scoring
except ImportError:
...
async def test_call_llm_for_dimension_success():
    ...
    with patch("veritas_news.ai.bias_analysis.asyncio.to_thread", side_effect=mock_to_thread):
        with patch("veritas_news.ai.bias_analysis._call_gemini_sync", return_value="5"):
...
async def test_rate_bias_parallel_success():
    ...
    with patch(
        "veritas_news.ai.bias_analysis.call_llm_for_dimension", side_effect=mock_llm_call
    ):
...
async def test_rate_bias_function():
    ...
    with patch("veritas_news.ai.config.get_prompts_config") as mock_config:
        with patch("veritas_news.ai.bias_analysis.rate_bias_parallel") as mock_parallel:
            with patch("veritas_news.ai.scoring.score_bias") as mock_scoring:
                ...
                try:
                    ...
                    from veritas_news.ai.bias_analysis import rate_bias
