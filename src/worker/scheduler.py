...
class JobScheduler:
        ...
        while self.running:
            try:
                ...
                if articles:
                    ...
                    logger.info(f"{job_name} job completed: {len(stored_ids)} articles stored")
                else:
                ...
                if sleep_time > 0:
                    logger.info(f"{job_name} job sleeping for {sleep_time:.1f}s until next run")
                    await asyncio.sleep(sleep_time)
                else:
                    logger.warning(f"{job_name} job took longer than interval ({elapsed:.1f}s > {interval}s)")
    ...
    async def start(self):
        ...
        try:
            ...
            main_interval = self.config.get("newsapi", {}).get("interval", WorkerConfig.DEFAULT_POLL_INTERVAL)
    ...
    async def run_single_fetch(self):
        ...
        try:
            ...
            if articles:
                stored_ids = self.pipeline.process_articles(articles)
                logger.info(f"Single fetch completed: {len(stored_ids)} articles stored")
                return stored_ids
            else:
