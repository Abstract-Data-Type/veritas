...
class TestApplicationStartup:
    ...
    def test_main_module_imports_successfully(self):
        """Test that src.main can be imported without syntax errors"""
        try:
            # This will fail if there are syntax errors in main.py
            import src.main
            assert hasattr(src.main, 'app'), "FastAPI app instance should be available"
    ...
    def test_lifespan_function_exists(self):
        ...
        assert hasattr(src.main, 'lifespan'), "lifespan function should exist"
        
        # Test that lifespan is callable (it's an async context manager decorator)
        import inspect
        assert callable(src.main.lifespan), "lifespan should be callable"
        
        # Check that it has the @asynccontextmanager decorator behavior
        lifespan_func = src.main.lifespan
