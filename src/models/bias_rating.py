...
class BiasRatingBase(BaseModel):
    ...
    bias_score: Optional[float] = Field(None, ge=-1.0, le=1.0, description="Bias score between -1 (left bias) and 1 (right bias)")
    reasoning: Optional[str] = Field(None, description="Reasoning behind the bias rating")
...
class BiasRatingResponse(BiasRatingBase):
    ...
    article_id: int = Field(..., description="ID of the article being rated")
    evaluated_at: datetime = Field(..., description="Timestamp when the rating was evaluated")
