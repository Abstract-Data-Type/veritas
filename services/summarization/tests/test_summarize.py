...
def test_llm_failure_returns_502(mock_summarize):
    ...
    mock_summarize.side_effect = HTTPException(
...
def test_success_returns_summary(mock_summarize):
    ...
    resp = client.post(
        "/summarize",
        json={"article_text": "This is a long article about important news events."},
    )
...
def test_gemini_integration_success(mock_client_class):
    ...
    resp = client.post("/summarize", json={"article_text": "Article content here"})
...
def test_gemini_api_error_returns_502(mock_client_class):
    ...
    resp = client.post("/summarize", json={"article_text": "Article content"})
