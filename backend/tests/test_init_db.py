...
from veritas_news.db.init_db import get_connection, init_db
from veritas_news.db.sqlalchemy import Base
from veritas_news.models.sqlalchemy_models import (
    Article,
    Summary,
    User,
    UserInteraction,
...
class TestInitDb:
    ...
    def test_init_db_creates_all_tables(self):
        ...
        (
            init_db()
        )  # This will use the global engine, but we'll inspect our test engine
    ...
    def test_init_db_foreign_key_constraints(self):
        ...
        )  # Should have 2 foreign keys (user_id and article_id)
        ...
        try:
            ...
            for action in valid_actions:
                interaction = UserInteraction(
                    user_id=user.user_id, article_id=article.article_id, action=action
                )
                db.add(interaction)
            db.commit()
    ...
    def test_init_db_with_error_handling(self):
        ...
        with patch("veritas_news.db.init_db.Base.metadata.create_all") as mock_create:
...
class TestIntegration:
    ...
    def test_full_database_initialization(self):
        ...
        try:
            ...
            article = Article(
                title="Test Article", source="Test Source", url="http://example.com"
            )
            db.add(article)
            db.commit()
            db.refresh(article)
            ...
            retrieved_article = (
                db.query(Article).filter(Article.title == "Test Article").first()
            )
            ...
            summary = Summary(
                article_id=article.article_id, summary_text="This is a test summary"
            )
            db.add(summary)
            db.commit()
            ...
            retrieved_summary = (
                db.query(Summary)
                .filter(Summary.article_id == article.article_id)
                .first()
            )
